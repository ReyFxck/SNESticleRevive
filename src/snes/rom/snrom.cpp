...
void SnesRom::SetCartInfo(SNRomInfoT *pCartInfo)
{
	...
	if (pCartInfo)
	{
		...
		if (pCartInfo->SRAMSize == 0)
			...
			m_uSRAMSize = 0;
		/* 34h/35h overlap ordinary decimal type values, so SA-1 is selected
		   only when the cartridge map byte is the defining mode 23h. */
...
Emu::Rom::LoadErrorE SnesRom::LoadRom(CDataIO *pFileIO, Uint8 *pBuffer, Uint32 nBufferBytes)
{
...
#if SNDBG_LOG
	{
		const char *pMapper =
			...
			m_eMapping == SNROM_MAPPING_EXHIROM ? "ExHiROM" :
			m_eMapping == SNROM_MAPPING_SA1 ? "SA-1" : "unknown";
