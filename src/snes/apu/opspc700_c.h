...
SNSPC_OP(0x07a,5)
	...
	SNSPC_GET_YA16(t2);
	{
		Uint32 uYA = t2 & 0xFFFF;
		Uint32 uValue = t1 & 0xFFFF;
		Uint32 uLowCarry = (((uYA & 0xFF) + (uValue & 0xFF)) > 0xFF);
		Uint32 uResult = uYA + uValue;
		r_P &= ~(SNSPC_FLAG_V | SNSPC_FLAG_H);
		if (((((uYA >> 8) & 0x0F) + ((uValue >> 8) & 0x0F) + uLowCarry) & 0x10) != 0)
			r_P |= SNSPC_FLAG_H;
		if ((~(uYA ^ uValue) & (uYA ^ uResult) & 0x8000) != 0)
			r_P |= SNSPC_FLAG_V;
		t2 = uResult;
	}
	SNSPC_SET_YA16(t2);
...
SNSPC_OP(0x09a,5)
	...
	SNSPC_GET_YA16(t2);
	{
		Uint32 uYA = t2 & 0xFFFF;
		Uint32 uValue = t1 & 0xFFFF;
		Uint32 uLowBorrow = ((uYA & 0xFF) < (uValue & 0xFF));
		Uint32 uHighA = (uYA >> 8) & 0xFF;
		Uint32 uHighB = (uValue >> 8) & 0xFF;
		Uint32 uResult = (uYA - uValue) & 0xFFFF;
		r_P &= ~(SNSPC_FLAG_V | SNSPC_FLAG_H);
		fC = (uYA >= uValue) ? 1 : 0;
		if ((((uHighA & 0x0F) - (uHighB & 0x0F) - uLowBorrow) & 0x10) == 0)
			r_P |= SNSPC_FLAG_H;
		if (((uYA ^ uValue) & (uYA ^ uResult) & 0x8000) != 0)
			r_P |= SNSPC_FLAG_V;
		t2 = uResult;
