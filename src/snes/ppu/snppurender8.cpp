...
static void _FetchCHR2_64(const Uint16 *pVram, Uint32 uBaseAddr, const SnesRenderTileT *pTiles, Int32 nTiles, Uint32 uScrollY, Uint8 *pDest, Uint8 *pMask, Uint64 *pPalLookup)
{
	...
	while (nTiles > 0)
	{
		...
		Uint32 uRowKey = (uRowAddr & 0x7FFFu) |
			((pTiles->uFlip & 1u) << 15);
		Uint64 uTile0;
		Uint32 uMask;

		if (SnesPPUBGAddrCacheLookup(&cache, uRowKey, &uTile0, &uMask))
		{
#if SNDBG_LOG
			g_DbgBGChrRepeatRows++;
			g_DbgBGAddrCacheHits++;
#endif
		}
		else
		{
#if SNDBG_LOG
			g_DbgBGAddrCacheMisses++;
#endif
			...
			SnesPPUBGAddrCacheStore(&cache, uRowKey, uTile0, uMask);
		}
	...
	while (nTiles > 0)
	{
		...
		else
		{
			...
			const SnesChrLookup64T *pLookup =
				(const SnesChrLookup64T *)pFlip->pLookup;
			Uint8 *pHFlip = pFlip->pFlipLookup;
			Uint32 uPlane0 = pTile8->uPlane01[0][0];
			Uint32 uPlane1 = pTile8->uPlane01[0][1];
			Uint32 uPlane2 = pTile8->uPlane23[0][0];
			Uint32 uPlane3 = pTile8->uPlane23[0][1];
			Uint32 uPlane4 = pTile8->uPlane45[0][0];
			Uint32 uPlane5 = pTile8->uPlane45[0][1];
			Uint32 uPlane6 = pTile8->uPlane67[0][0];
			Uint32 uPlane7 = pTile8->uPlane67[0][1];
			Uint32 uPlanes = uPlane0 | uPlane1 | uPlane2 | uPlane3 |
