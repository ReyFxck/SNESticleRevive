...
// --- SNESticleRevive integration -------------------------------------------
// Entry points + globals (originally extern'd by the x86 asm glue dsp4proc.c,
// which is dropped here in favour of the SNDSP4 C++ bus wrapper in sndsp4.cpp).
extern uint8_t  dsp4_byte;     // byte exchanged with the bus (in/out)
extern uint16_t dsp4_address;  // (unused by the wrapper; kept for parity)

void InitDSP4(void);    // reset DSP4 + DSP4_vars
void DSP4SetByte(void); // feed one written byte (in dsp4_byte) to the FSM
void DSP4GetByte(void); // fetch one output byte (result in dsp4_byte)
