...
/*
 * Adapted for SNESticleRevive (2026, ReyFxck) from the ZSNES dsp4emu.c:
 *  - Replaced the unaligned pointer-cast buffer macros (READ_WORD/READ_DWORD/
 *    WRITE_WORD/WRITE_DWORD) with alignment-safe little-endian byte access,
 *    so the parameter/output buffers work on the PS2's MIPS R5900 (which
 *    faults on unaligned 16/32-bit loads).
 *  - Added the 0x14 "Test ROM Version" command (returns 0x0400 = DSP-4).
 *  - InitDSP4() now also clears DSP4_vars.
 *  - Compiled as C++ and driven by the SNDSP4 bus wrapper (sndsp4.cpp)
 *    instead of the original x86 inline-asm glue (dsp4proc.c, dropped).
 * The DSP-4 projection math itself is unchanged from the ZSNES original.
 */
...
// Alignment-safe little-endian access (SNESticleRevive): the original ZSNES
// macros cast the byte buffer to uint16_t*/uint32_t*, which faults on MIPS
// (PS2) for unaligned offsets. These helpers assemble/emit bytes explicitly.
static inline uint16_t dsp4_rd_word(const uint8_t* s) {
    return (uint16_t)(s[0] | ((uint16_t)s[1] << 8));
}
static inline uint32_t dsp4_rd_dword(const uint8_t* s) {
    return (uint32_t)(s[0] | ((uint32_t)s[1] << 8) | ((uint32_t)s[2] << 16) | ((uint32_t)s[3] << 24));
}
static inline void dsp4_wr_word(uint8_t* s, uint16_t d) {
    s[0] = (uint8_t)(d & 0xFF);
    s[1] = (uint8_t)(d >> 8);
}
static inline void dsp4_wr_dword(uint8_t* s, uint32_t d) {
    s[0] = (uint8_t)(d & 0xFF);
    s[1] = (uint8_t)(d >> 8);
    s[2] = (uint8_t)(d >> 16);
    s[3] = (uint8_t)(d >> 24);
}
#define READ_WORD(s) dsp4_rd_word((const uint8_t*)(s))
#define READ_DWORD(s) dsp4_rd_dword((const uint8_t*)(s))
#define WRITE_WORD(s, d) dsp4_wr_word((uint8_t*)(s), (uint16_t)(d))
#define WRITE_DWORD(s, d) dsp4_wr_dword((uint8_t*)(s), (uint32_t)(d))
...
void DSP4_OP09()
{
    ...
    resume1:
        ...
        resume6:
            ...
            if (DSP4_vars.sprite_clipy - pixels <= sp_y && sp_y <= DSP4_vars.sprite_clipy && sp_x >= DSP4_vars.viewport_left - pixels && sp_x <= DSP4_vars.viewport_right && DSP4_vars.sprite_clipy >= DSP4_vars.viewport_top - pixels && DSP4_vars.sprite_clipy <= DSP4_vars.viewport_bottom) {
                DSP4_OP0B(&draw, sp_x, DSP4_vars.sprite_clipy, 0x00EE, DSP4_vars.sprite_size, 0);
            }

            // normal sprite tile
            if (sp_x >= DSP4_vars.viewport_left - pixels && sp_x <= DSP4_vars.viewport_right && sp_y >= DSP4_vars.viewport_top - pixels && sp_y <= DSP4_vars.viewport_bottom && sp_y <= DSP4_vars.sprite_clipy) {
...
void InitDSP4()
{
    ...
    memset(&DSP4_vars, 0, sizeof(DSP4_vars));
    DSP4.waiting4command = true;
}
