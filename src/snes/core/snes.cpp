...
Uint32 g_DbgBGRawCacheHits = 0;
Uint32 g_DbgBGRawCacheMisses = 0;
...
static void SnesDbgResetWindow(void)
{
	...
	g_DbgBGRawCacheHits = 0;
	g_DbgBGRawCacheMisses = 0;
...
void SnesSystem::ExecuteFrame(Emu::SysInputT  *pInput, CRenderSurface *pTarget, CMixBuffer *pSound, ModeE eMode)
{
	...
	// --- timing: fecha o frame e resume a janela ---
	{
		...
		if (g_TmgWinFrames >= SNDBG_FRAME_PERIOD)
		{
			...
			DLog("[snes-diag] schema=%s level=%u session=%u window=%u inclusive-timing=1 rom-rules=0 bg-cache=raw-vram-4slot obj-cache=%u",
			...
			DLog("[snes-bg-raw] cache hit/miss=%u/%u slots=4",
				(unsigned)g_DbgBGRawCacheHits,
				(unsigned)g_DbgBGRawCacheMisses);
			...
			DLog("[snes-obj-cache] bytes=149504 invalidated-slots=%u bg-path=raw-vram",
