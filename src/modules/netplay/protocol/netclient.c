...
/* Hugo's upstream netclient.c chains a series of `} else <comment>\n
   if (...)` branches inside _NetClientSocketRead.  This is logically
   an `else if` ladder but GCC 13+ flags the cross-line indentation as
   `-Wmisleading-indentation`.  Suppress the warning here so the file
   stays byte-for-byte identical to its source in
   hugorsgarcia/PS2SNESticle/SNESticle/Modules/netplay/Source/common
   instead of churning the vendored code purely for cosmetics. */
